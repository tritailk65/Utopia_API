...
public class FollowingService implements IFollowingService{
    ...
    public List<Following> getAllFollowingByUser(@Nullable Long id) {
        ...
        List<Following> followById = _followingRepo.findAllByUserSourceId(id);
    ...
    @Override
    public Boolean cancelFollow(Long userSrc, Long userTar) {
        try {
//          #region: Check data
            if(_followingRepo.findFollowingByUserSourceId(userSrc)){
                throw new ResourceNotFoundException("ID User source sai, kiểm tra lại");
            }
            
            if(_followingRepo.findFollowingByUserTargetId(userTar)){
